...
pub fn run2() -> usize {
    let guards = get_guards();
    let guard = guards.iter().max_by_key(|guard| guard.max_minute().1).unwrap();
    guard.id * guard.max_minute().0

}

fn get_guards() -> Vec<Guard> {
    let mut input: Vec<String> = read_file(INPUT).split('\n').into_iter().map(|s| s.to_string()).collect();
    ...
    let r = input.into_iter().scan(0, |state, e| {
        *state = match2.captures(e.as_str())
            .map(|e| e.get(1).unwrap().as_str().parse::<usize>().unwrap())
            .unwrap_or(*state);

        let time = match1.captures(e.as_str())
            .map(|e| (e.get(1).unwrap().as_str().parse::<usize>().unwrap(), e.get(2).unwrap().as_str().parse::<usize>().unwrap())).unwrap();
    ...
    r.chunks(2).map(|c| {
        (c[0].0, (c[0].1).1..(c[1].1).1)
    }).fold(HashMap::<usize, Vec<usize>>::new(), |mut acc, e| {
        acc.entry(e.0).or_insert(vec![]).append(&mut e.1.collect::<Vec<_>>());
        acc
    }).iter().map(|(k, v)| Guard::new(*k,v.clone())).collect()
}